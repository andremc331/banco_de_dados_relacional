...
CREATE OR REPLACE FUNCTION log_atualiza_qtd_projetos()
RETURNS TRIGGER AS
$$
BEGIN
...
CREATE TRIGGER trg_atualiza_qtd_projetos
AFTER INSERT OR UPDATE OR DELETE
ON tbl_departamentos
FOR EACH ROW
EXECUTE FUNCTION log_atualiza_qtd_projetos();

--2 crie uma trigger que atualiza a quantidade de projetos da tbl_departamentos caso um novo projeto for adicionado

CREATE OR REPLACE FUNCTION log_atualiza_qtd_projetos_if_add()
RETURNS TRIGGER AS
$$
BEGIN
...
CREATE TRIGGER trg_atualiza_qtd_projetos_if_add
AFTER INSERT OR UPDATE OR DELETE
ON tbl_departamentos
FOR EACH ROW
EXECUTE FUNCTION log_atualiza_qtd_projetos_if_add();
...
CREATE OR REPLACE FUNCTION log_salva_salario_atual()
RETURNS TRIGGER AS
$$
BEGIN
...
CREATE TRIGGER trg_salva_salario_atual
AFTER UPDATE OF salario
ON tbl_empregados
FOR EACH ROW
EXECUTE FUNCTION log_salva_salario_atual();
...
CREATE OR REPLACE FUNCTION log_atualiza_data()
RETURNS TRIGGER AS
$$
BEGIN 
...
AFTER UPDATE OF data_atualizacao
ON tbl_empregados
FOR EACH ROW
EXECUTE FUNCTION log_atualiza_data();